...
/**
 * A generic, self-contained head writer.
 *
 * The marketplace pages own their own metadata (facet titles, listing
 * descriptions, dynamic canonicals) and cannot go through the static
 * ROUTE_META table, so they call this directly. It writes exactly the
 * same tags usePageMeta writes, which keeps every route in the app
 * consistent — and lets scripts/prerender.mjs stamp byte-compatible raw
 * HTML for the crawl-first pass.
 */
export interface HeadMeta {
  title: string;
  description: string;
  /** Absolute canonical URL. `null` removes the element (noindex pages). */
...
export function applyHeadMeta(meta: HeadMeta): void {
  const { title, description, canonical, robots = null } = meta;
  const url = meta.url ?? canonical ?? `${SITE_URL}/`;
...
// Marketplace listings carry schema.org Vehicle + BreadcrumbList JSON-LD.
// Their own element (own id, own owner tag) so it can never collide with
// the car JSON-LD above, and it is removed again when the visitor leaves
// a listing page.
const MARKETPLACE_JSONLD_ID = "marketplace-jsonld";

export function syncMarketplaceJsonLd(data: unknown[] | null): void {
  const existing = document.getElementById(MARKETPLACE_JSONLD_ID) as HTMLScriptElement | null;
  if (!data || !data.length) {
    existing?.remove();
    return;
  }
  const target =
    ...
    (() => {
      ...
      script.id = MARKETPLACE_JSONLD_ID;
      script.type = "application/ld+json";
      script.dataset.owner = "marketplace";
...
export function isShellMetaRoute(path: string): boolean {
  ...
  // Marketplace pages (list + facets + listing detail + sell) and the
  // private admin dashboard write their own head: facet canonicals and
  // listing metadata are dynamic, and /marketplace/sell + /admin/* must
  // stay noindex. They are intentionally absent from ROUTE_META.
  if (p.startsWith("/marketplace") || p.startsWith("/admin")) return false;
...
export function usePageMeta({ car, story, notFound, path, skip }: PageMeta) {
  useEffect(() => {
    ...
    applyHeadMeta({ title, description, canonical, url, image, type, robots });
